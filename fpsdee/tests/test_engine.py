...
from fpsdee import config, engine
...
class CommandBuilders(unittest.TestCase):
    def setUp(self):
        self.settings = {"tools": {"ffmpeg": "", "ffprobe": "", "deew_python": "/venv/python", "deezy": "/venv/deezy",
                                   "mediainfo": "", "truehdd": ""}}
    ...
    def test_deew_argv(self):
        ...
        self.assertEqual(engine.deew_cmd_encode("/w/t.wav", e, "bogus", "/w")[5:10], ["-f", "ddp", "-b", "1536", "-r"])
        self.assertIn("film_light", engine.deew_cmd_encode("/w/t.wav", e, "bogus", "/w"))   # unknown DRC falls back

    def test_deezy_argv(self):
        e = engine.resolve_encode("ddp", 0, True, 0, 8, True, "truehd")
        self.assertEqual(engine.deezy_cmd_atmos("/in/m.mkv", 1, e, "film_standard", "/w", "/o/m.ec3"),
                         ["/venv/deezy", "encode", "atmos", "--atmos-mode", "bluray", "--bitrate", "1536",
    ...
    def test_deew_output_accepts_eb3(self):
        with tempfile.TemporaryDirectory() as tmp:
            self.assertIsNone(engine._deew_output(tmp, "temp_extract"))
            open(os.path.join(tmp, "temp_extract.eb3"), "w").close()
            self.assertEqual(engine._deew_output(tmp, "temp_extract"), os.path.join(tmp, "temp_extract.eb3"))
    ...
    def test_nested_encode_settings_roundtrip(self):
        with tempfile.TemporaryDirectory() as tmp:
            with mock.patch.object(config, "config_dir", return_value=__import__("pathlib").Path(tmp)):
                ...
                self.assertEqual(config.load_settings()["tools"]["deezy"], "/x/deezy")
